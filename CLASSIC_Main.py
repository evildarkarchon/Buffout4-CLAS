...
def get_file_paths():
    file_paths = {
        ...
        "docs_file_papyrus": yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Docs_File_PapyrusLog"),
        "docs_file_wrye": yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Docs_File_WryeBashPC"),
        "docs_file_xse": yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Docs_File_XSE"),
        "docs_path": yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Root_Folder_Docs"),
        "docs_folder_xse": yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Docs_Folder_XSE"),
        "docs_file_papyruslog": yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Docs_File_PapyrusLog"),
    }
    return file_paths
if Path(f"CLASSIC Data/CLASSIC {game} Local.yaml").exists():
...
def docs_path_find():
    ...
    def get_windows_docs_path():
        ...
        win_docs = os.path.join(documents_path, f"My Games\\{game_settings["docs_name"]}")

        # Update the YAML settings (assuming this function exists)
        file_paths["docs_path"] = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Root_Folder_Docs", f"{win_docs}")
        ...
        if libraryfolders_path.is_file():
            ...
            for library_line in steam_library:
                ...
                if str(game_sid) in library_line:
                    ...
                    linux_docs = library_path.joinpath("compatdata", str(game_sid), "pfx", "drive_c", "users", "steamuser", "My Documents", "My Games", game_settings["docs_name"])
                    file_paths["docs_path"] = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Root_Folder_Docs", f"{linux_docs}")

    def get_manual_docs_path():
        print(f"> > > PLEASE ENTER THE FULL DIRECTORY PATH WHERE YOUR {game_settings["docs_name"]}.ini IS LOCATED < < <")
        while True:
            path_input = input(f"(EXAMPLE: C:/Users/Zen/Documents/My Games/{game_settings["docs_name"]} | Press ENTER to confirm.)\n> ")
            if Path(path_input).exists() and Path(path_input).is_dir():
                ...
                file_paths["docs_path"] = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Root_Folder_Docs", manual_docs)
                break
            else:
    ...
    if file_paths["docs_path"] is None:
        if platform.system() == "Windows":
    ...
    try:  # In case .exists() complains about checking a None value.
        if not Path(file_paths["docs_path"]).exists(): # type: ignore
...
def docs_generate_paths():
    ...
    file_paths["docs_folder_xse"] = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Docs_Folder_XSE", fr"{file_paths["docs_path"]}\{file_paths["xse_acronym_base"]}")
    file_paths["docs_file_papyruslog"] = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Docs_File_PapyrusLog", fr"{docs_path}\Logs\Script\Papyrus.0.log")
