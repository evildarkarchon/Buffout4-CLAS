...
def classic_generate_files() -> None:  # Other paths will be auto generated by the code.
    ...
    local_path = Path(f"CLASSIC Data/CLASSIC {game} Local.yaml")
    ...
    fid_mods_path = Path(f"CLASSIC Data/databases/{game} FID Mods.txt")
    if not fid_mods_path.exists():
        default_fidfile: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", "Default_FIDMods") # type: ignore
    ...
    if journal_path.exists():
        ...
        if log_age.days > 7:
            try:
                ...
                print(f"{journal_path.name} has been deleted and regenerated due to being older than 7 days.")
                logging.basicConfig(
                    level=logging.INFO,
...
def create_formid_db() -> None:
    formid_db_path = Path(f"CLASSIC Data/databases/{game} FormIDs.db")
    with sqlite3.connect(formid_db_path) as conn:
        ...
        if not formid_db_path.exists() or not formid_db_path.stat().st_size > 0:
            ...
            batch_insert_entries_from_file(Path(f"CLASSIC Data/databases/{game} FID Main.txt"), formid_db_path)
            print(" Done!")
...
def classic_data_extract() -> None:
    ...
    fid_main_path = Path(f"CLASSIC Data/databases/{game} FID Main.txt")
    try:
    ...
    if fid_main_path.exists() and not Path(f"CLASSIC Data/databases/{game} FormIDs.db").exists():
        create_formid_db()
...
def docs_path_find() -> None:
    ...
    docs_name: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.Main_Docs_Name")  # type: ignore
    ...
    def get_windows_docs_path() -> None:
        ...
        yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Root_Folder_Docs", str(win_docs))

    def get_linux_docs_path() -> None:
        game_sid: int = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.Main_SteamID")  # type: ignore
    ...
    def get_manual_docs_path() -> None:
        ...
        while True:
            ...
            if input_str and input_path.is_dir():
                ...
                yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Root_Folder_Docs", str(input_path))
                break
    ...
    docs_path: str | None = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Root_Folder_Docs")  # type: ignore
...
def docs_generate_paths() -> None:
    ...
    xse_acronym: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.XSE_Acronym")  # type: ignore
    xse_acronym_base: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", "Game_Info.XSE_Acronym")  # type: ignore
    docs_path: str = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Root_Folder_Docs")  # type: ignore
...
def game_path_find() -> None:
    ...
    xse_file: str = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Docs_File_XSE")  # type: ignore
    xse_acronym: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.XSE_Acronym")  # type: ignore
    xse_acronym_base: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", "Game_Info.XSE_Acronym")  # type: ignore
    game_name: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.Main_Root_Name")  # type: ignore
...
def game_generate_paths() -> None:
    ...
    game_path: str = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Root_Folder_Game")  # type: ignore
    xse_acronym: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.XSE_Acronym")  # type: ignore
    xse_acronym_base: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", "Game_Info.XSE_Acronym")  # type: ignore
...
def game_check_integrity() -> str:
    ...
    steam_ini_local: str = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Game_File_SteamINI")  # type: ignore
    exe_hash_old: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.EXE_HashedOLD")  # type: ignore
    # exe_hash_new: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.EXE_HashedNEW")  # type: ignore  | RESERVED FOR 2023 UPDATE
    game_exe_local: str = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Game_File_EXE")  # type: ignore
    root_name: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.Main_Root_Name")  # type: ignore
...
def xse_check_integrity() -> str:  # RESERVED | NEED VR HASH/FILE CHECK
    ...
    xse_acronym: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.XSE_Acronym") # type: ignore
    xse_log_file: str = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Docs_File_XSE") # type: ignore
    xse_full_name: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.XSE_FullName") # type: ignore
    xse_ver_latest:str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.XSE_Ver_Latest") # type: ignore
    adlib_file: str = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Game_File_AddressLib") #type: ignore
...
def xse_check_hashes() -> str:
    ...
    xse_hashedscripts: dict[str, str] = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.XSE_HashedScripts") # type: ignore
    game_folder_scripts: str = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Game_Folder_Scripts") # type: ignore
...
def docs_check_ini(ini_name: str) -> str:
    ...
    folder_docs: str = yaml_settings(f"CLASSIC Data/CLASSIC {game} Local.yaml", f"Game{vr}_Info.Root_Folder_Docs")  # type: ignore
    docs_name: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", f"Game{vr}_Info.Main_Docs_Name")  # type: ignore
    ...
    if any(ini_name.lower() in file.name.lower() for file in ini_file_list):
        ...
        if ini_name.lower() == f"{docs_name.lower()}custom.ini":
            with ini_path.open("a", encoding="utf-8", errors="ignore") as ini_file:
                message_list.extend(["❌ WARNING : Archive Invalidation / Loose Files setting is not enabled. \n",
                                     "  CLASSIC will now enable this setting automatically in the game INI files. \n-----\n"])
                customini_config: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", "Default_CustomINI")  # type: ignore
...
def main_generate_required() -> None:
    ...
    game_name: str = yaml_settings(f"CLASSIC Data/databases/CLASSIC {game}.yaml", "Game_Info.Main_Root_Name")  # type: ignore
